...
const StructurePanel = dynamic(() => import("./structure/StructurePanel"), { ssr: false });

export type Project = { id: string; kind: string; title: string | null; description: string | null };
    ...
    <div className="h-[calc(100vh-0px)] grid grid-cols-12 gap-4 p-4">
      ...
      <div className="col-span-12 lg:col-span-6 rounded-xl bg-white dark:bg-neutral-900 shadow-sm border flex flex-col">
        <div className="p-3 border-b flex items-center justify-between">
          <div className="flex items-center gap-2 font-medium"><BookOpenText size={16}/> {project.title ?? "Untitled Project"}</div>
      ...
      <div className="lg:hidden col-span-12 grid grid-cols-3 gap-2">
        <button onClick={()=> setActiveTab("chat")} className={`p-2 rounded-lg border ${activeTab==='chat'?'bg-[#F5B942] text-[#1C2B3A]':''}`}>Chat</button>
