...
export default function StructurePanel({
  ...
  const isNodeAChapter = (node: Node): boolean => {
    ...
    const draftFolder = nodes.find(
      (n) => n.name === "draft" && n.kind === "folder" && !n.parent_id
    );
    ...
    while (current) {
      if (
        current.parent_id === draftFolder.id ||
        current.id === draftFolder.id
      ) {
        return true;
      }
  ...
  const createNode = async (kind: NodeKind) => {
    ...
    try {
      ...
      const untitledName =
        kind === "folder" ? "Untitled Folder" : "Untitled File";
  ...
  const renameNode = async (nodeId: string) => {
    ...
    const res = await fetch(
      `/api/projects/${projectId}/nodes?nodeId=${nodeId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
            ...
            >
              {renamingId === n.id ? (
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") renameNode(n.id);
                  }}
                ...
                <>
                  {n.kind === "folder" ? (
                    <Folder
                      size={16}
                      className="text-[#4BB3A4] flex-shrink-0"
                    />
                  ) : isNodeAChapter(n) ? (
                    <BookOpen
                      size={16}
                      className="text-[#F5B942] flex-shrink-0"
                    />
                  ) : (
                    <File
                      size={16}
